                    ...
                    switch value {
                        case ChatMessage.Init:
                            ...
                            do {
                                ...
                                // Mando mensaje banned al que voy a eliminar
                                var BannedBuffer = Data(capacity: 1000)
                                withUnsafeBytes(of: ChatMessage.Banned) { BannedBuffer.append(contentsOf: $0) }

                                do {
                                    try self.serverSocket.write(from: BannedBuffer, to: first!.addres)
                                    BannedBuffer.removeAll()
                                } catch {
                                    print("Error al mandar Welcome")
                                }
                                ...
                                func sendAll(client: Client) {
                                    ...
                                    do {
                                        if client.addres != clientAddress! && client.addres != first!.addres {
                                            try self.serverSocket.write(from: sendBuffer, to: client.addres)
                                        }
