    ...
    repeat {
        let (bytesRead, clientAddress) = try serverSocket.readDatagram(into: &buffer)
                if let address = clientAddress {
                    ...
                    // Calculo el tamaño que vot a leer
                    count = MemoryLayout<Point>.size
                    // Copio los bytes de nuevo porque no puedo acceder a un lugar de la memoria
                    bytesCopied = withUnsafeMutableBytes(of: &point) {
                        buffer.copyBytes(to: $0, from: offset..<offset+count) // Rango del buffer adecuado
                    }
                    assert(bytesCopied == count)
                    offset += count
