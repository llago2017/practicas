        ...
        do {
            ...
            func sendInit() {
                do {
                    ...
                    print("Attempting connection, timeout: \(attempt)s")
                    
                    // Preparo timeout para recibir welcome
                    try clientSocket.setReadTimeout(value: UInt(attempt * 1000))
                    let (bytesRead, _) = try clientSocket.readDatagram(into: &buffer)

                    if bytesRead == 0 && errno == EAGAIN { 
                        // Si no ha llegado a tiempo vuelvo a intentar doblando el tiempo
                        if times < 4 {
                            attempt = attempt * 2
                            times += 1
                            sendInit()
